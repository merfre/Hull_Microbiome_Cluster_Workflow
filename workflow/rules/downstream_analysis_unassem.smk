...
rule alpha_diversity_unassem:
  ...
  output:
    report = report("results/downstream/alpha_div_table_unassem.tsv", caption="report/alpha_diversity_reports.rst", category="Downstream Analyses")
  params:
...
rule beta_diversity_unassem:
  ...
  input:
    "results/biom/kraken2_unassem_allsamples.tsv"
  output:
    report = report("results/downstream/beta_pca_unassem.pdf", caption="report/beta_diversity_reports.rst", category="Downstream Analyses")
  params:
    tax_level = config['taxonomy_level']
  script:
...
rule tax_heatmap_unassem:
  ...
  input:
    "results/biom/kraken2_unassem_allsamples.tsv"
  output:
    report = report("results/downstream/tax_heatmap_unassem.pdf", caption="report/tax_heatmap_reports.rst", category="Downstream Analyses")
  params:
    tax_level = config['taxonomy_level']
  script:
...
input:
  "results/biom/kraken2_unassem_allsamples.tsv"
output:
  report = report("results/downstream/tax_barplot_unassem.pdf", caption="report/tax_barplot_reports.rst", category="Downstream Analyses")
params:
  tax_level = config['taxonomy_level']
script:
